...
llm = ChatOpenAI(temperature=0, model="gpt-4", api_key=os.getenv("OPEN_AI_API_KEY"), streaming=True)
...
async def start():
        ...
        for i, chunk in enumerate(chunks):
            processed_docs.append(
                Document(
                    page_content=chunk,
                    metadata={
                        "source": doc.metadata.get("source", f"document_{documents.index(doc)}"),
    ...
    docsearch = await cl.make_async(FAISS.from_documents)(processed_docs, embedding_function)
...
# Query complexity detection prompt
...
async def main(message: cl.Message):
    ...
    if query_complexity == "simple":
        ...
        final_answer_prompt = PromptTemplate(
            template="Based on the following sub-questions and answers, provide the direct answer to the main question: {main_question}\n\nSub-questions and answers:\n{combined_answer}\n\nUse Pointer to present your answer where you feel necessary or in a clean way. Don't provide unnecessary information. Final answer:",
    ...
    for doc in source_documents:
        source_key = (doc.metadata['source'], doc.metadata.get('page', 'unknown'))
        if source_key not in unique_sources:
            unique_sources[source_key] = doc

    # Prepare source citations
    source_citations = []
    for source_key in unique_sources.keys():
        citation = f"{source_key[0]}#page={source_key[1]}"
        source_citations.append(citation)

    # Create a single Text element for all sources
    if source_citations:
        sources_content = "\n".join(source_citations)
        sources_element = cl.Text(content=sources_content, name="Sources")
        
        # Add a note about sources to the final answer
        final_answer += f"\n\nThis answer is based on {len(source_citations)} source(s)."
    else:
        sources_element = None
        final_answer += "\nNo specific sources found for this answer."

    # Send the final answer with the sources element
    await cl.Message(
        content=final_answer,
        elements=[sources_element] if sources_element else []
    ).send()
